...
def run_forecast_benchmarks():
    """Run all forecasting benchmarks."""
    ...
    for res_dim in RES_DIMS:
        ...
        reservoir = Reservoir(
            lr=leak_rate,
            units=res_dim,
            Win=esn.embedding.win[0],
            W=esn.driver.wr[0],
            bias=np.ones(res_dim) * bias,
        )
        rpy_esn = (reservoir >> Ridge(beta)).fit(
            U_fcast[:-1], U_fcast[1:], warmup=spinup
        )
        ...
        print(
            f"  [forecast] res_dim={res_dim}: ORC {orc_fcast_times[-1]:.4f} ms/step, "
...
def run_training_benchmarks():
    ...
    for n_steps in TRAIN_SIZES:
        ...
        _e, _s = orc.forecaster.train_RCForecaster(
            esn, U_train, beta=beta, spinup=spinup
        )
        ...
        for _ in range(N_REPEATS):
            t0 = time.perf_counter()
            _, rs = orc.forecaster.train_RCForecaster(
                esn, U_train, beta=beta, spinup=spinup
            )
        ...
        print(
            f"  [training] n_steps={n_steps}: ORC {orc_train_times[-1]:.3f} s, "
...
ax1.plot(
    ...
    rpy_fcast_cpu,
    "s--",
    color=COLORS["rpy_cpu"],
    markersize=6,
    linewidth=1.5,
    label="ReservoirPy (CPU)",
)
ax1.plot(
    RES_DIMS,
    rpy_fcast_gpu,
    "D--",
    color=COLORS["rpy_gpu"],
    markersize=6,
    linewidth=1.5,
    label="ReservoirPy (GPU)",
)
