...
def test_lorenz63_custom_args():
    ...
    us, ts = orc.data.lorenz63(tN, dt, u0=u0, rho=rho, sigma=sigma, beta=beta, solver=solver, stepsize_controller=stepsize_controller, saveat=saveat)
...
def test_lorenz63_symmetry():
    """Test the symmetry of the lorenz63 integrator. If (u1,u2,u3) is a solution, then (-u1,-u2,u3) should also be a solution."""
    tN = 10.0
    dt = 0.01
    Nt = int(tN / dt)
...
def test_rossler_custom_args():
    ...
    us, ts = orc.data.rossler(tN, dt, u0=u0, a=a, b=b, c=c, solver=solver, stepsize_controller=stepsize_controller, saveat=saveat)
...
def test_sakaraya_custom_args():
    ...
    us, ts = orc.data.sakaraya(tN, dt, u0=u0, a=a, b=b, m=m, solver=solver, stepsize_controller=stepsize_controller, saveat=saveat)
...
def test_colpitts_custom_args():
    ...
    us, ts = orc.data.colpitts(tN, dt, u0=u0, alpha=alpha, gamma=gamma, q=q, eta=eta, solver=solver, stepsize_controller=stepsize_controller, saveat=saveat)
...
def test_hyper_lorenz63_custom_args():
    ...
    us, ts = orc.data.hyper_lorenz63(tN, dt, u0=u0, a=a, b=b, c=c, d=d, solver=solver, stepsize_controller=stepsize_controller, saveat=saveat)
...
def test_hyper_xu_custom_args():
    ...
    us, ts = orc.data.hyper_xu(tN, dt, u0=u0, a=a, b=b, c=c, d=d, e=e, solver=solver, stepsize_controller=stepsize_controller, saveat=saveat)
...
def test_double_pendulum_custom_args():
    ...
    us, ts = orc.data.double_pendulum(tN, dt, u0=u0, m1=m1, m2=m2, L1=L1, L2=L2, g=g, damping=damping, solver=solver, stepsize_controller=stepsize_controller, saveat=saveat)
...
def test_lorenz96_custom_args():
    ...
    us, ts = orc.data.lorenz96(tN, dt, u0=u0, N=N, F=F, solver=solver, stepsize_controller=stepsize_controller, saveat=saveat)
...
def test_KS_1D_custom_args():
    ...
    assert U.shape == (Nt, Nx + 1), f"Expected U shape ({Nt}, {Nx + 1}), but got {U.shape}"
    assert t.shape == (Nt,), f"Expected t shape ({Nt},), but got {t.shape}"
...
def test_KS_1D_conservation():
    ...
    assert jnp.isclose(conserved_quantity, 0.0, atol=1e-10), f"Conserved quantity is not close to zero: {conserved_quantity}"
