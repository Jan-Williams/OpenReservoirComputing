...
@pytest.fixture(params=[(50, 0.1), (100, 0.02), (1000, 0.01), (2000, 0.01)])
def matrix_params(request):
    return request.param


def random_sparse_matrix(n, density):
    key = jax.random.PRNGKey(999)
    A = jax.experimental.sparse.random_bcoo(
        key=key,
        shape=(n, n),
        nse=density,
        dtype=jnp.float64,
...
def test_max_eig_diag(matrix_params):
...
def test_max_eig_random_sparse(matrix_params):
    """Test max_eig_arnoldi with a random sparse matrix."""
    n, density = matrix_params
...
def test_max_eig_seed_consistency(matrix_params):
