...
[DT: is DL Deep learning? I would define that in the caption]
...
[DT: Another thing that I don't think should be understated here is the power of `vmap` and that ORC was built to be fully compatible with it from the ground up. This means things like ensembles and hyperparameter opt could be implemented very efficiently and easily. I would even consider adding "Vectorization" as a column in the table]

# Software Design

![ORC three-layer pipeline architecture. \label{fig:architecture}](../imgs/architecture.svg) [DT: this is a nice summary! only feedback here is that the rendered math font is really wonky. Subscripts look big and misaligned. Not sure if its a markdown thing, but worth double checking.]
...
ORC supports parallel reservoirs [DT: cite parallel RC paper] by default via a `chunks` parameter in each module. This slightly complicates built-in training support and introduces an extra tensor dimension that can make the API less intuitive at first. However, we believe this tradeoff is worthwhile because it enables spatiotemporal RC methods unavailable in other libraries. [DT: Don't we also have a batch dimension to avoid nuking the GPU's VRAM? might be worth mentioning here to avoid confusion.]
...
![ORC with GPU acceleration enables significantly faster performance than ReservoirPy, even when using ReservoirPy's JAX backend. Performance of the two libraries with and without GPU acceleration are shown. GPU results were obtained running on an NVIDIA A40 GPU and CPU results were obtained with an Apple M2 chip. \label{fig:comp}](../imgs/benchmark_results.png) [DT: Have you benchmarked this against Julia? Curious how ORC stacks up. Also, what about training time?]
...
Claude (Anthropic) was used for code assistance during code debugging, proofreading this paper, and the generation of the ORC logo [DT: technically this was ChatGPT] and architecture visualization SVG. All generated code was reviewed, tested, and validated by the authors.
