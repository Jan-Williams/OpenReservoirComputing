...
ORC addresses an immediate need for RC researchers. It allows for easy reimplementation of architectures such as [cite magri, williams, others] that integrate reservoir computing with larger neural network architectures, and makes it easier to iterate on these ideas than any existing library. We also benchmark ORC against ReservoirPy across reservoir sizes in \autoref{fig:comp}. We find that particularly with GPU acceleration, ORC scales far better than ReservoirPy. Moreover, ORC is the only current package that supports training RC for control tasks, thanks to its efficiency and the end-to-end differentiability that JAX and Equinox provide. RC-based control has recently been shown to be advantageous over other popular forms of RNN-based MPC. ORC's performance also makes it well suited for the parallel architectures needed to extend RC to higher-dimensional settings.
